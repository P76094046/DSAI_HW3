...
data_ = scaler.fit_transform(data.values)
gen_train = data_[:a, 0]
gen_train = np.expand_dims(gen_train, axis = 1)
gen_train_label = label[:a, 0]

con_train = data_[:a, 1]
con_train = np.expand_dims(con_train, axis = 1)
con_train_label = label[:a, 1]
gen_val = data_[a:, 0]
con_val = data_[a:, 1]
#%%
...
class dataset(torch.utils.data.Dataset):     # for training set
    ...
    def __getitem__(self, index):
        X = self.data[index : index + num_4_pred]
        y = self.label[index + num_4_pred : index + num_4_pred + num_pred]
        X = torch.from_numpy(X).type(torch.Tensor)
        y = torch.from_numpy(y).type(torch.Tensor)
        return X, y
    
batch_size = 16
num_epochs = 1 #n_iters / (len(train_X) / batch_size)

gen_dataset = dataset(gen_train, gen_train_label)
con_dataset = dataset(con_train, con_train_label)
gen_loader = torch.utils.data.DataLoader(dataset = gen_dataset,#dataset 
...
for i in range(23):
    gen_normalize = np.append(gen_normalize[1:167], np.array((pred.cpu().detach().numpy() - mean_gen) / std_gen))
