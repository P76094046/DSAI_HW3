...
if __name__ == "__main__":
    ...
    for i in range(24):
        gen_norm = np.append(gen_norm[1:167], np.array((pred_gen.cpu().detach().numpy() - mean_gen) / std_gen))
        gen = data_processor2(gen_norm, mean_gen, std_gen)
        pred_gen = model_gen(gen)
        pred_gen_list.append(pred_gen)
    
        con_norm = np.append(con_norm[1:167], np.array((pred_con.cpu().detach().numpy() - mean_con) / std_con))
